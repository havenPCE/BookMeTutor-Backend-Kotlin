...
class BookmetutorApplicationTests {
        ...
        assertAll(
                {
                    ...
                    assertEquals("UPDATE METHOD SUCCESS", updatedQualification, tutorQualificationRepo.update(updatedQualification))
                    assertEquals("UPDATE METHOD FAILURE", null, tutorQualificationRepo.update(falseQualification))
                    assertEquals("FIND BY TUTOR SUCCESS", updatedQualification, tutorQualificationRepo.findByTutorId(tutorId))
        ...
        assertAll(
                {
                    ...
                    assertEquals("FIND BY TUTOR METHOD SUCCESS", true, bookingRepo.findByTutorId(tutorId).contains(updatedBooking))
                    assertEquals("FIND BY TUTOR METHOD FAILURE", false, bookingRepo.findByTutorId(falseTutorId).contains(updatedBooking))
                    assertEquals("FIND BY STUDENT ID METHOD SUCCESS", true, bookingRepo.findByStudentId(studentId).contains(updatedBooking))
                    assertEquals("FIND BY STUDENT ID FAILURE", false, bookingRepo.findByStudentId(falseStudentId).contains(updatedBooking))
    ...
    fun `Test For TutorRepo`() {
        ...
        assertAll({
            ...
            assertEquals("FIND SUITABLE M1 SUCCESS", updatedTutor, tutorRepo.findTutorForAssignment(Gender.MALE, "ROURKELA", rejects = emptyList()))
            assertEquals("FIND SUITABLE M1 FAILURE", maleTutor2, tutorRepo.findTutorForAssignment(Gender.MALE, "ROURKELA", rejects = listOf(updatedTutor.email)))
            assertEquals("FIND SUITABLE F1 SUCCESS", femaleTutor1, tutorRepo.findTutorForAssignment(Gender.FEMALE, "ROURKELA", rejects = emptyList()))
            assertEquals("FIND SUITABLE F1 FAILURE", null, tutorRepo.findTutorForAssignment(Gender.FEMALE, "ROURKELA", rejects = listOf(femaleTutor1.email, femaleTutor2.email)))
