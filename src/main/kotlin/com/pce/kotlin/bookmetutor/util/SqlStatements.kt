...
object SubjectQuery {
    ...
    fun insertIntoSubject(subject: Subject) = Pair(
        ...
        MapSqlParameterSource(
            mutableMapOf(
                "id" to subject.id,
                "subjectName" to subject.subjectName.name,
                "classNumber" to subject.classNumber
            )
        )
    )

    fun insertIntoTopic(subjectId: Long) =
        "INSERT INTO public.subject_topic(subject_id, topic) VALUES (${subjectId},:topic);"
...
object TutorQualificationQuery {
    ...
    fun insert(tutorId: Long, qualification: TutorQualification) = Pair(
        ...
        MapSqlParameterSource(
            mutableMapOf(
                "id" to qualification.id,
                "degree" to qualification.degree,
                "university" to qualification.university,
                "percentile" to qualification.percentile,
                "tutorId" to tutorId
            )
        )
    )
    ...
    fun insert(bookingId: Long, invoice: Invoice) = Pair(
        ...
        MapSqlParameterSource(
            mutableMapOf(
                ...
                "method" to invoice.method.name,
                "summary" to invoice.summary,
                "bookingId" to bookingId
            )
        )
    )
...
object BookingQuery {
    ...
    fun insertIntoReject(bookingId: Long) =
        """INSERT INTO public.booking_reject(booking_id, reject) VALUES (${bookingId}, :reject);"""
    ...
    fun insertIntoTopic(bookingId: Long) =
        """INSERT INTO public.booking_topic(booking_id, topic) VALUES (${bookingId}, :topic);"""
...
object StudentQuery {
    ...
    fun insertIntoPhone(studentId: Long) =
        """INSERT INTO public.student_phone(student_id, phone) VALUES (${studentId},:phone)"""
